...
A4・1枚あたりの実測（CPU 4コア、検証用PDF）。

| `--dpi` | 1枚あたり | 判定結果 | 平均確信度 |
|--------:|----------:|----------|-----------:|
| 150 | 6.3s | 5件とも正しい | 0.87 |
| 200（既定） | 12.2s | 5件とも正しい | 0.84 |
| 300 | 14.2s | 5件とも正しい | 0.84 |

モデルの読み込みは起動時の1.7秒だけで、2枚目以降は上表の時間しかかからない。
100枚なら150dpiで約10分、200dpiで約20分。

**上げても精度は上がらない。** 150dpiの方が確信度がむしろ高いくらいで、
時間だけが倍になる。まず150で試し、細かい明細行を取りこぼすようなら
200、それでも駄目なら300、という順で上げるのが速い。
なお検証用PDFの本文は12pt相当なので、これより小さい文字の原稿では
必要な解像度が変わる。`--dump-text` で実際の読み取り結果を見て決めるとよい。
